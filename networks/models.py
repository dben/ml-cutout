...
class MattingModule(nn.Module):
    ...
    def forward(self, image, two_chan_trimap, image_n, trimap_transformed):
        ...
        :param two_chan_trimap: input trimap
        :param image_n: input image_n
        :param trimap_transformed: input trimap_transformed
        :return: mask
        """
...
class ResnetDilated(nn.Module):
    ...
    def forward(self, x, return_feature_maps=False):
        """
        :param x: input image
        :param return_feature_maps: if True, return the feature maps
...
def fba_fusion(alpha, img, F, B):
    """
    Fusion of the foreground and background.
    :param alpha: foreground mask
...
class fba_decoder(nn.Module):
    ...
    def forward(self, conv_out, img, indices, two_chan_trimap):
        """
        :param conv_out: list of feature maps from backbone
        :param img: input image
        :param indices: indices of the maxpooling
        :param two_chan_trimap: two channel trimap
        :return: fused image
        """
        conv5 = conv_out[-1]
